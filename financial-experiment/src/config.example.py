...
DATA_DIR        = ROOT / "data"
ARTICLES_PATH   = DATA_DIR / "news" / "articles.json"
RESPONSES_PATH  = DATA_DIR / "raw" / "responses.jsonl"
RESULTS_PATH    = DATA_DIR / "structured" / "results.csv"
FIGURES_DIR     = DATA_DIR / "figures"

# ── API ────────────────────────────────────────────────────────────────────
# Set OPENROUTER_API_KEY in your shell before running:
#   export OPENROUTER_API_KEY=sk-or-...
OPENROUTER_API_KEY = os.environ.get("OPENROUTER_API_KEY")
OPENROUTER_BASE_URL = "https://openrouter.ai/api/v1"

# ── Models ─────────────────────────────────────────────────────────────────
# Fill in SMALL_MODEL_ID with any OpenRouter model ID before running.
...
# Analysis also requires a minimum share of rows with both mandatory extracted fields
MANDATORY_FIELD_QUALITY_THRESHOLD = 0.90


def require_openrouter_api_key():
    """Return the API key or raise a clear error before live API calls."""
    if OPENROUTER_API_KEY:
        return OPENROUTER_API_KEY

    raise RuntimeError(
        "OPENROUTER_API_KEY is not set. Export it in your shell before running "
        "generator.py, extractor.py, or test_run.py."
    )
