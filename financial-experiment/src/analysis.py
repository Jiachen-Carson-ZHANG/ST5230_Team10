...
Computes six modular metrics. Saves outputs to data/figures/.
...
from src.config_runtime import (
    ...
    FIGURES_DIR,
    COVERAGE_THRESHOLD,
    MANDATORY_FIELD_QUALITY_THRESHOLD,
)


MANDATORY_FIELDS = ("risk_rating_score", "strategic_action")

STRATEGIC_POSTURE_BY_ACTION = {
    # Decisive directional actions collapse into one aggressive posture bucket.
    "Strong_Buy": "Aggressive",
    "Clear_Short": "Aggressive",
    # Friend's doc distinguishes "reduce exposure" from pure observation.
    "Reduce_Exposure": "Defensive",
    "Halt_Compliance": "Defensive",
    "Hold_Monitor": "Observational",
}

ACTION_URGENCY_SCORES = {
    # The friend's examples anchor "immediate action" at 5 and near-term monitoring at 2.
    "Immediate": 5,
    "Short_Term": 2,
    "Long_Term": 1,
}
...
def load_and_validate():
    ...
    enforce_mandatory_field_quality_gate(merged)

    return merged


def enforce_mandatory_field_quality_gate(
    df, threshold=MANDATORY_FIELD_QUALITY_THRESHOLD
):
    """
    Require a minimum share of rows to contain both mandatory extraction fields.
    Aborts analysis when extraction quality is too degraded to trust the metrics.
    """
    if df.empty:
        return 1.0

    complete_mask = df[list(MANDATORY_FIELDS)].notna().all(axis=1)
    completeness_rate = float(complete_mask.mean())
    complete_rows = int(complete_mask.sum())
    total_rows = len(df)

    print(
        "\nMandatory field completeness:",
        f"{complete_rows}/{total_rows} rows ({completeness_rate:.1%})",
    )

    if completeness_rate < threshold:
        print(
            f"Mandatory extraction completeness {completeness_rate:.1%} "
...
def with_strategic_posture(df):
    """Add a coarse strategic_posture column derived from strategic_action."""
    if "strategic_posture" in df.columns:
        return df

    df = df.copy()
    df["strategic_posture"] = df["strategic_action"].map(STRATEGIC_POSTURE_BY_ACTION)
    return df


def with_action_urgency_score(df):
    """Add a numeric urgency score derived from the extracted urgency enum."""
    if "action_urgency_score" in df.columns:
        return df

    df = df.copy()
    df["action_urgency_score"] = df["action_urgency"].map(ACTION_URGENCY_SCORES)
    return df


def export_unsupported_claim_cases(df):
    """Write all flagged unsupported-claim rows for manual review."""
    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    export_path = FIGURES_DIR / "unsupported_claim_cases.csv"
    ...
    available_columns = [col for col in case_columns if col in df.columns]
    cases = df.loc[df["hallucination"], available_columns].copy()
    cases.to_csv(export_path, index=False)
...
def compute_flip_rate(df):
    """
    Flip rate: % of samples where strategic_posture differs from the mode (most common),
    grouped by article × persona × model.
    Higher = more unstable decision-making across repeated runs.
    """
    print("\n=== Flip Rate ===")
    df_clean = with_strategic_posture(df).dropna(subset=["strategic_posture"])

    def flip_rate_group(group):
        mode = group["strategic_posture"].mode()
        if mode.empty:
            return np.nan
        modal_value = mode.iloc[0]
        return (group["strategic_posture"] != modal_value).mean()

    result = (
        df_clean
        .groupby(["article_id", "persona_id", "model"])
        .apply(flip_rate_group)
    ...
    ax.set_title("Mean Strategic Posture Flip Rate by Persona and Model")
    fig.tight_layout()
    fig.savefig(FIGURES_DIR / "strategic_posture_flip_rate_heatmap.png", dpi=150)
    plt.close(fig)
    print("Saved: strategic_posture_flip_rate_heatmap.png")
    return result
...
def compute_action_urgency_score(df):
    """
    Action urgency score: numeric urgency derived from the extracted urgency label,
    grouped by persona × model.
    Higher score = the persona/model combination recommends faster action.
    """
    print("\n=== Action Urgency Score ===")
    df_clean = with_action_urgency_score(df).dropna(subset=["action_urgency_score"])
    ...
    ax.set_title("Mean Action Urgency Score by Persona and Model")
    fig.tight_layout()
    fig.savefig(FIGURES_DIR / "action_urgency_score_heatmap.png", dpi=150)
    plt.close(fig)
    print("Saved: action_urgency_score_heatmap.png")
    return result
...
def compute_unsupported_claim_rate(df):
    ...
    export_unsupported_claim_cases(df_clean)
    return result
